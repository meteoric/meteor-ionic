...
function($scope
  ...
  self._handleDrag = function(e) {
    if (isAsideExposed || !$scope.get('dragContent')) return;
  ...
  self.canDragContent = function(canDrag) {
    if (arguments.length) {
      $scope.set('dragContent', !!canDrag);
    }
    return $scope.get('dragContent');
  };
  ...
  self.isDraggableTarget = function(e) {
    ...
    return ($scope.get('dragContent') || self.isOpen()) &&
      dragIsWithinBounds &&
  ...
  $scope.set('sideMenuContentTranslateX', 0);

  var deregisterBackButtonAction = () => {};
  var closeSideMenu = self.close.bind(self);

  Tracker.autorun((c) => {
    let isOpen = self.getOpenAmount() !== 0;
    if (isOpen) {
    ...
    $(self).on('$destroy', function() { c.stop(); });
  });
