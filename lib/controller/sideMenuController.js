meteoric.controller.ionicSideMenus =
function() {
  //$ionicHistory,
  var self = this;
  ...
  let $ionicScrollDelegate = meteoric.controller.ionicScrollDelegate;
  let $ionicSideMenuDelegate = meteoric.service.ionicSideMenuDelegate;
  let $ionicPlatform = meteoric.service.ionicPlatform();
  let $ionicBody = meteoric.service.ionicBody();
  let IONIC_BACK_PRIORITY = meteoric.service.IONIC_BACK_PRIORITY;

  self.initialize = function($scope) {
    self.$scope = $scope;

    // To get around $watch
    self._openAmount = new ReactiveVar(0);

    self._initialize = function (options) {
      ...
      self._openAmount.set(self.getOpenAmount());
    };
    ...
    self.toggleLeft = function (shouldOpen) {
      ...
      if (!shouldOpen) {
        ...
        $(self).trigger('$ionicSideMenuClose', ['left']);
      } else {
        self.openPercentage(100);
        $(self).trigger('$ionicSideMenuOpen', ['left']);
      }
    };
    ...
    self.toggleRight = function (shouldOpen) {
      ...
      if (!shouldOpen) {
        self.openPercentage(0);
        $(self).trigger('$ionicSideMenuClose', ['right']);
      } else {
        self.openPercentage(-100);
        $(self).trigger('$ionicSideMenuOpen', ['right']);
      }
    };
    ...
    self.exposeAside = function (shouldExposeAside) {
      ...
      $(self).trigger('$ionicExposeAside', [isAsideExposed]);
    };
    ...
    self.isDraggableTarget = function (e) {
      ...
      let backView = true;  // todo: Remove this when ionicHistory is implemented.
    ...
    var deregisterBackButtonAction = () => {
    };
    var closeSideMenu = self.close.bind(self);

    // To get around $watch
    Tracker.autorun((c) => {
      let isOpen = self._openAmount.get();
      if (isOpen) {
    ...
    $ionicSideMenuDelegate.addAggregate(this);
    ...
    $($scope).on('$destroy', function () {
      $ionicSideMenuDelegate.removeAggregate(this);
